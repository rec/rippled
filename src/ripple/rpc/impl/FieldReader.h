...
struct FieldReader
{
    ...
    FieldReader (Json::Value& params_) : params (params_) {}
};

/** Read a required field of generic type T from a FieldReader.
...
/** Read an optional field of generic type T from a FieldReader.
...
/**
   Try to read a ledger from a FieldReader.

   Return true on success;  on failure, fill FieldReader::error and
   return false.
*/
bool readLedger (FieldReader&, Ledger::pointer& result, NetworkOPs&);

/**
   Try to read an account from a string.

   Return true on success;  on failure, fill error and return false.
*/
bool readAccount (
    Json::Value& error, AccountID& result, std::string const& accountName);

bool readAccountAddress (FieldReader&, AccountID&);
