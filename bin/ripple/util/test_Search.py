...
class test_Search(TestCase):
    ...
    def test_linear_full(self):
        self.assertEquals(list(linear_search(0, 20, self.condition)),
                          [10, 11, 12, 13, 14])

    def test_linear_partial(self):
        self.assertEquals(list(linear_search(8, 13, self.condition)),
                          [10, 11, 12, 13])
        self.assertEquals(list(linear_search(11, 13, self.condition)),
                          [11, 12, 13])
        self.assertEquals(list(linear_search(12, 17, self.condition)),
                          [12, 13, 14])

    def test_linear_empty(self):
        self.assertEquals(list(linear_search(1, 3, self.condition)), [])
